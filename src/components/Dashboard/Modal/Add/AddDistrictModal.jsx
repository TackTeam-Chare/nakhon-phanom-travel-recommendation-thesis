...
const AddDistrictModal = ({ isOpen, onClose, onDistrictAdded }) => {
  ...
  const onSubmit = async (data) => {
    try {
      // Call createDistrict function to add a new district
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
                  <div className="relative">
                    <input
                      ...
                      placeholder="ชื่อเขต"
                      className={`w-full border ${errors.name ? "border-red-500" : "border-gray-300"} rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-orange-500`}
                    />
                    {errors.name && (
